...
import com.moez.qksms.databinding.MainPermissionHintBinding
import com.moez.qksms.databinding.MainSyncingBinding
...
class MainActivity : QkThemedActivity(), MainView {
    ...
    private var hintBinding: MainPermissionHintBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.snackbar.setOnInflateListener { _, inflated ->
            hintBinding = MainPermissionHintBinding.bind(inflated)
            hintBinding?.snackbarButton?.clicks()
    ...
    override fun render(state: MainState) {
        ...
        when (state.syncing) {
            ...
            is SyncRepository.SyncProgress.Running -> {
                ...
                syncingBinding?.syncingProgress?.let { progressView ->
                    progressView.max = state.syncing.max
                    ObjectAnimator.ofInt(progressView, "progress", progressView.progress, state.syncing.progress).start()
                    progressView.isIndeterminate = state.syncing.indeterminate
                }
        ...
        when {
            !state.defaultSms -> {
                hintBinding?.snackbarTitle?.setText(R.string.main_default_sms_title)
                hintBinding?.snackbarMessage?.setText(R.string.main_default_sms_message)
                hintBinding?.snackbarButton?.setText(R.string.main_default_sms_change)
            }

            !state.smsPermission -> {
                hintBinding?.snackbarTitle?.setText(R.string.main_permission_required)
                hintBinding?.snackbarMessage?.setText(R.string.main_permission_sms)
                hintBinding?.snackbarButton?.setText(R.string.main_permission_allow)
            }

            !state.contactPermission -> {
                hintBinding?.snackbarTitle?.setText(R.string.main_permission_required)
                hintBinding?.snackbarMessage?.setText(R.string.main_permission_contacts)
                hintBinding?.snackbarButton?.setText(R.string.main_permission_allow)
            }

            !state.notificationPermission -> {
                hintBinding?.snackbarTitle?.setText(R.string.main_permission_required)
                hintBinding?.snackbarMessage?.setText(R.string.main_permission_notifications)
                hintBinding?.snackbarButton?.setText(R.string.main_permission_allow)
            }
        }
    }
